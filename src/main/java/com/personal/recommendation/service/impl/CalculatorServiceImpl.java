...
public class CalculatorServiceImpl implements CalculatorService {
    ...
    @Override
    public BaseRsp executeInstantJob(List<Long> userIdList, int type) {
        // 验证参数
        if(!RecommendationUtil.paramFilter(userIdList, type)){
        ...
        // 基于内容推荐需要刷新用户preference
        if (type == RecommendationEnum.CB.getCode())
            refresh(userIdList);
    ...
    public void refresh(List<Long> userIdsCol) {
        ...
        HashMap<Long, ArrayList<Long>> userBrowsedMap = getBrowsedHistoryMap(userIdsCol);
        ...
        HashMap<Long, CustomizedHashMap<Integer, CustomizedHashMap<String, Double>>> userPrefListMap =
                usersManager.getUserPrefListMap(new ArrayList(userBrowsedMap.keySet()));

        // 新闻对应关键词列表与模块ID
        HashMap<String, Object> newsTFIDFMap = getNewsTFIDFMap(userIdsCol, userBrowsedMap);
     ...
     */
    @Override
    public HashMap<Long, ArrayList<Long>> getBrowsedHistoryMap(List<Long> userList) {
        HashMap<Long, ArrayList<Long>> userBrowsedMap = new HashMap<>();
        try {
            List<NewsLogs> newsLogsList = newsLogsManager.getNewsLogsByUsersViewTime(
                    DateUtil.getDateBeforeDays(RecommendationConstants.HOT_DATA_DAYS), userList);
     ...
     */
    @Override
    public List<Long> getBrowsedNewsSet(HashMap<Long, ArrayList<Long>> browsedMap) {
     ...
     */
    @Override
    public HashMap<String, Object> getNewsTFIDFMap(List<Long> userList, HashMap<Long, ArrayList<Long>> browsedMap) {
        HashMap<String, Object> newsTFIDFMap = new HashMap<>();
        try {
            // 提取出所有新闻的关键词列表及对应TF-IDf值，并放入一个map中
            List<News> newsList = newsManager.getNewsByIds(getBrowsedNewsSet(browsedMap));
            for (News news : newsList) {
                newsTFIDFMap.put(String.valueOf(news.getId()), TfIdf.getTfIde(news.getTitle(),
                        news.getContent(), RecommendationConstants.KEY_WORDS_NUM));
