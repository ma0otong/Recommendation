...
public interface RecommendationsDAO {
    ...
    @Select("select * from " + TABLE + " where user_id = #{userId} and feedback = 0 order by derive_time desc limit #{limit}")
    List<Recommendations> getNewsByUserId(@Param("userId") long userId, @Param("limit") int limit);
